...
public:
	...
	template <class T> void Storage(T *begin, T *end, GLbitfield flags) {
		glNamedBufferStorage(id_, (end - begin) * sizeof(T), begin, flags);
	}

	// never use Data() with Storage()
	template <class T> void Data(T *begin, T *end, GLenum usage) {
		glNamedBufferData(id_, (end - begin) * sizeof(T), begin, usage);
	}

	template <class T> void SubData(GLsizei byte_offset, const T *begin, const T *end) {
